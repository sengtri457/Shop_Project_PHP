...
class Variant
{
    ...
    public static function findBySkus(array $skus): array
    {
        if (empty($skus)) {
            return [];
        }
        ...
        $stmt = $db->prepare("SELECT * FROM product_variants WHERE sku IN ($placeholders)");
        $stmt->execute($skus);
    ...
    public static function update(int $id, array $data): void
    {
        ...
        foreach (['sku', 'price', 'stock_qty', 'attributes', 'image_url'] as $field) {
            if (array_key_exists($field, $data)) {
                $fields[] = "$field = ?";
                $params[] = $field === 'attributes' ? json_encode($data[$field]) : $data[$field];
            }
        }
